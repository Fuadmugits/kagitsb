...
function calculateTotalStats(jid, chatJid = null) {
    ...
    const ascLuckMult = 1 + ((userRpg.asc_luck || 0) * 0.1);
    ...
    // Title Bonus (Multiplies total final stats)
    if (userRpg.equipped_title) {
        const title = RPG_TITLES.find(t => t.id === userRpg.equipped_title);
        if (title && title.stats) {
            totalStats.power = Math.floor(totalStats.power * (title.stats.power || 1));
            totalStats.defense = Math.floor(totalStats.defense * (title.stats.defense || 1));
            totalStats.luck = Math.floor(totalStats.luck * (title.stats.luck || 1));
            totalStats.expMult = title.stats.exp || 1.0;
        }
    }
