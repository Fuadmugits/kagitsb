...
async function tick() {
        ...
        for (const sub of subs) {
            ...
            for (const pKey of PRAYER_KEYS) {
                ...
                try {
                    ...
                    if (!sub.jid.endsWith('@g.us') || Settings.get(`mute_${sub.jid}`) !== 'true') {
    ...
    try {
        ...
        for (const r of dues) {
            ...
            try {
                const { Settings } = require('../database');
                if (!r.chat_jid.endsWith('@g.us') || Settings.get(`mute_${r.chat_jid}`) !== 'true') {
    ...
    // ── 3. Global Game Schedule ───────────────────────
    try {
        const config = require('../config');
        if (config.gameSchedule && config.gameSchedule.length > 0) {
            for (const schedule of config.gameSchedule) {
                if (schedule.time === hhMM) {
                    const { Settings } = require('../database');
                    const currentState = Settings.get('global_game_active');
                    if (currentState !== schedule.state) {
                        Settings.set('global_game_active', schedule.state);
                        console.log(`🎮 Game Schedule triggered: ${hhMM} → Game is now ${schedule.state.toUpperCase()}`);
                    }
                    break;
                }
            }
        ...
        console.log('Scheduler game schedule error:', e.message);
    }
}
