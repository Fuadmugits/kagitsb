...
function werewolfAction(groupJid, werewolfJid, targetJid) {
    ...
    if (game.werewolfVotes.has(werewolfJid)) return { success: false, error: 'Kamu sudah memilih target malam ini! Tunggu malam berikutnya.' };
...
function seerAction(groupJid, seerJid, targetJid) {
    ...
    // 1x per night check
    if (game.nightActions.seerTarget !== null) return { success: false, error: 'Kamu sudah meramal malam ini! Tunggu malam berikutnya.' };
...
function doctorAction(groupJid, doctorJid, targetJid) {
    ...
    // 1x per night check
    if (game.nightActions.doctorTarget !== null) return { success: false, error: 'Kamu sudah melindungi seseorang malam ini! Tunggu malam berikutnya.' };
...
/**
 * Resolve target from a number index (1-based) in the alive player list
 * Also supports raw phone number input
 */
function resolveTargetByNumber(game, input, excludeJid = null) {
    if (!input) return null;
    const cleaned = input.toString().replace(/[^0-9]/g, '');
    if (!cleaned) return null;

    // Build alive list (same order as getAlivePlayerListText)
    const aliveList = [];
    for (const [jid, player] of game.players) {
        if (!player.alive) continue;
        if (excludeJid && jid === excludeJid) continue;
    ...
    // Try as index first (1, 2, 3...)
    const idx = parseInt(cleaned, 10);
    if (idx >= 1 && idx <= aliveList.length) {
        return aliveList[idx - 1];
    }

    // Try as phone number
    const asJid = cleaned + '@s.whatsapp.net';
    if (game.players.has(asJid)) return asJid;

    return null;
}

/**
 * Check if a player is dead in any active game in a specific group
 * Returns game object if player is dead, null otherwise
 */
function isPlayerDeadInGame(groupJid, playerJid) {
    const game = global.werewolfGames?.get(groupJid);
    if (!game || game.phase === PHASE.LOBBY || game.phase === PHASE.ENDED) return null;
    const player = game.players.get(playerJid);
