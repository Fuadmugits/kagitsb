...
async function handleMessage(sock, rawMsg, io) {
    try {
        ...
        if (command.ownerOnly && !isOwner(m.sender)) {
            // Debug log untuk diagnosa masalah grup @lid
            if (m.isGroup) {
                const { getNumberFromJid } = require('./functions');
                console.log(`[OWNER_CHECK] Grup sender JID: ${m.sender}`);
                console.log(`[OWNER_CHECK] Resolved number: ${getNumberFromJid(m.sender)}`);
                console.log(`[OWNER_CHECK] OWNER_NUMBER: ${config.bot.ownerNumber.join(', ')}`);
                console.log(`[OWNER_CHECK] raw participant: ${m.raw?.key?.participant}`);
            }
