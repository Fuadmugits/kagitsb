...
async function handleMessage(sock, rawMsg, io) {
    try {
        ...
        if (m.isGroup && !m.body.startsWith(config.bot.prefix) && Settings.get(`leveling_${m.chat}`) !== 'false') {
        ...
        if (command.category === 'games' && m.isGroup && Settings.get(`game_${m.chat}`) === 'false' && !isOwner(m.sender)) {
