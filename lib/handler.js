...
function executeCommandQueued(taskFn) {
    return new Promise((resolve, reject) => {
        if (commandQueue.length >= MAX_QUEUE_SIZE) {
            return reject(new Error('QUEUE_FULL'));
        }
        commandQueue.push({ taskFn, resolve, reject });
        processCommandQueue();
    });
}

function processCommandQueue() {
    if (activeCommandsCount >= MAX_CONCURRENT_COMMANDS || commandQueue.length === 0) {
        return;
    }
    const { taskFn, resolve, reject } = commandQueue.shift();
    activeCommandsCount++;
...
async function handleMessage(sock, rawMsg, io) {
    try {
        ...
        // Group Mute check
        if (m.isGroup && Settings.get(`mute_${m.chat}`) === 'true') {
            const isMuteCommand = m.body.toLowerCase().startsWith(config.bot.prefix + 'mute');
            if (!isOwner(m.sender) || !isMuteCommand) {
                return; // Ignore message if muted and not an owner sending a mute command
            }
        }
        ...
        if (m.isGroup && !isOwner(m.sender)) {
            ...
            const isBotAdmin = meta.participants.find(p => p.id === botJid)?.admin;
        ...
        if (Users.isJailed(m.sender) && !isOwner(m.sender)) {
                ...
                if (activeGame.answer !== undefined) {
                    ...
                    if (needsReply && (!m.quoted || m.quoted.key.id !== activeGame.msgId)) {
                        ...
                        if (answerLower === correctAnswer) {
                            let reward = activeGame.reward || 500;
                            ...
                            await sock.sendMessage(m.chat, {
                                text: `🎉 *BENAR!*\n\n✅ Jawaban: *${activeGame.answer}*\n💰 Hadiah: +${reward} balance ${multiplier > 1 ? `(Admin Abuse x${multiplier}! 🔥)` : ''}\n🏆 Selamat ${winner}!`,
                                mentions: [m.sender]
                            }, { quoted: m.raw });
                            global.activeGames.delete(m.chat);
                            return;
        ...
        // Cooldown / Anti-Spam Check
        const now = Date.now();
        const userCooldownKey = m.sender;
        const lastCmdTime = cooldowns.get(userCooldownKey) || 0;
        
        let cooldownDuration = 2000; // 2 seconds for Free users
        if (isOwner(m.sender)) {
            cooldownDuration = 0; // No cooldown for Owner
        } else if (Users.isPremium(m.sender)) {
            cooldownDuration = 800; // 0.8 seconds for Premium
        }

        if (cooldownDuration > 0 && (now - lastCmdTime) < cooldownDuration) {
            const timeRemaining = ((cooldownDuration - (now - lastCmdTime)) / 1000).toFixed(1);
            
            const lastWarnTime = lastWarnings.get(userCooldownKey) || 0;
            if (now - lastWarnTime > 5000) {
                lastWarnings.set(userCooldownKey, now);
                await m.reply(`⏳ *Anti-Spam!* Mohon tunggu *${timeRemaining} detik* sebelum menggunakan perintah lagi.`);
            }
            return;
        }
        ...
        // Global Game & Judi toggle (owner only toggle)
        if (command.category === 'games' && Settings.get('global_game_status') === 'false' && !isOwner(m.sender)) {
            return m.reply('🎮 Fitur game saat ini sedang *dinonaktifkan* secara global oleh Owner.');
        }
        if (command.category === 'judi' && Settings.get('global_judi_status') === 'false' && !isOwner(m.sender)) {
            return m.reply('🎲 Fitur judi saat ini sedang *dinonaktifkan* secara global oleh Owner.');
        }
        ...
        // Game schedule check (global)
        if (command.category === 'games' && Settings.get('global_game_active') === 'off' && !isOwner(m.sender)) {
            const isGroupGameOn = m.isGroup && Settings.get(`game_${m.chat}`) === 'true';
            if (!isGroupGameOn) {
                return m.reply('⏳ Fitur game saat ini *dinonaktifkan* sesuai jadwal.\nSilakan coba lagi nanti saat jam main!');
            }
        }
        ...
        // Execute command
        try {
            await executeCommandQueued(async () => {
                await command.execute({ sock, m, args, text, prefix, config, command });
            });
        } catch (err) {
            if (err.message === 'QUEUE_FULL') {
                return m.reply('⚠️ *Bot sedang sangat sibuk!*\n\nBanyak permintaan sedang masuk sekaligus. Silakan coba lagi beberapa saat lagi.');
            }
            throw err;
        }
