...
function serialize(msg, sock) {
    ...
    m.sender = m.fromMe
        ? (sock.user?.id ? (sock.user.id.includes('@lid') ? sock.user.id : sock.user.id.split(':')[0] + '@s.whatsapp.net') : 'bot@s.whatsapp.net')
        : m.isGroup
            ? (() => {
                const p = msg.key.participant || '';
                return p.includes('@lid') ? p : (p.split(':')[0] + '@s.whatsapp.net');
              })()
            : (m.chat.includes('@lid') ? m.chat : (m.chat.split(':')[0] + '@s.whatsapp.net'));
    ...
    if (msg.message) {
        ...
        if (contextInfo?.quotedMessage) {
            ...
            m.quoted.sender = contextInfo.participant
                ? (contextInfo.participant.includes('@lid') ? contextInfo.participant : contextInfo.participant.split(':')[0] + '@s.whatsapp.net')
                : '';
